...
def sign_up():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first()
        if user:
            flash('Email already exists.', category='error')  # Add your error handling here
        elif len(email) < 4:
            flash('Email must be greater than 3 characters.', category='error')  # Add your error handling here
        elif len(first_name) < 2:
            flash('First name must be greater than 1 character.', category='error')   # Add your error handling here
        elif password1 != password2:
            flash('Passwords don\'t match.', category='error')   # Add your error handling here
        elif len(password1) < 7:
            flash('Password must be atleast 7 characters.', category='error')   # Add your error handling here
        else:
            #create a new user
